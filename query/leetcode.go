...
func FetchLatestRecord(db *sql.DB) int {
	var total int
...
func UpdateOrCreateRecord(db *sql.DB, total int) {
	q := `INSERT INTO records(total) VALUES ($1)`
	_, err := db.Exec(q, total)
	if err != nil {
		log.Fatalf("Error occurred on Records table during insert new total record: %s\n", err.Error())
	}
	fmt.Printf("Latest record has been updated\n")
}

func InsertTag(db *sql.DB, tag model.TopicTag) {
	q := `INSERT INTO tags(id, name) VALUES ($1, $2) ON CONFLICT DO NOTHING`
	_, err := db.Exec(q, tag.ID, tag.Name)
	if err != nil {
		log.Fatalf("Error occurred on Tag table during insertion at %s, %s: %s\n", tag.ID, tag.Name, err.Error())
	}
	fmt.Printf("Insert Tag record: %s\n", tag.Name)
}

func InsertQuestion(db *sql.DB, question model.Question) {
	...
	if err != nil {
		log.Fatalf("Error occurred on Question table during insertion at No. - %s, with title - %s: %s\n", question.FrontendQuestionID, question.Title, err.Error())
	}
	fmt.Printf("Insert Question record: %s - %s\n", question.FrontendQuestionID, question.Title)
}

func InsertQuestionTag(db *sql.DB, questionId int, tag string) {
	q := `INSERT INTO questions_tags(question_id, tag_id) VALUES ($1, $2) ON CONFLICT DO NOTHING`
	_, err := db.Exec(q, questionId, tag)
	if err != nil {
		log.Fatalf("Error occurred on QuestionTag table during insertion at Question No. - %d, and Tag No. - %s: %s\n", questionId, tag, err.Error())
	}
